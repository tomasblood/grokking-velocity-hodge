...
\newcommand{\addedtext}[1]{\textcolor{blue}{#1}}
\input{generated/eight_seed_hodge_macros.tex}
\input{generated/eight_seed_hodge_baseline_rows.tex}
\input{generated/eight_seed_hodge_phase_sensitivity_rows.tex}
\pdfstringdefDisableCommands{\def\addedtext#1{#1}}
\theoremstyle{remark}
...
\begin{abstract}
Grokking refers to the phenomenon of neural networks hitting near-perfect training accuracy early and then taking orders of magnitude more steps to generalise on a held-out dataset \citep{power2022grokking}. \addedtext{The primary contribution is a pointwise velocity-field Hodge decomposition across eight grokking runs. Across eight training seeds, the baseline mean exact, coexact, and harmonic fractions change from $(\HodgeFixedTransitionExact,\HodgeFixedTransitionCoexact,\HodgeFixedTransitionHarmonic)$ during the fixed transition window to $(\HodgeFixedPostExact,\HodgeFixedPostCoexact,\HodgeFixedPostHarmonic)$ post-grokking. At the baseline setting, the post-minus-transition change in coexact-minus-exact fraction is $\HodgeFixedChange$ (95\% seed-level $t$ interval $[\HodgeFixedChangeLow,\HodgeFixedChangeHigh]$). Its direction is the same in all \HodgeSensitivityComparisons{} correlated seed-by-setting sensitivity comparisons and survives seed-relative phase alignment ($\HodgeEventChange$, 95\% interval $[\HodgeEventChangeLow,\HodgeEventChangeHigh]$). Permuting point correspondence removes the post-grokking exact enrichment. Effective dimensionality and resolvent BW distance provide global corroborating diagnostics. The robust result is therefore a post-grokking shift toward exact velocity, rather than a universal coexact signature during the transition itself.}
\end{abstract}
...
\addedtext{The primary contribution is a pointwise velocity-field Hodge decomposition across eight grokking runs. The transition is clearly visible in the representation and in the global corroborating diagnostics, while the completed seed, probe, parameter, synthetic-field, and correspondence-null checks delimit the Hodge claim. Supplementary geometric diagnostics and an audit of the evidential status of these claims appear in the appendix.}

\addedtext{In this paper we study the grokking transition through a primary spatial analysis and complementary global diagnostics.} Both are built on a shared representation space made from activation checkpoints.

\begin{enumerate}
    \item \addedtext{Primary spatial analysis.} The probe displacement field $v_i(t) = x_i(t + \Delta t) - x_i(t)$ between consecutive checkpoints is a vector field on the point cloud at time~$t$. We treat it as a degree-1 object and perform Hodge decomposition via DiffusionGeometry, partitioning the velocity into exact (gradient), coexact (curl), and harmonic components.

    \item \addedtext{Global corroborating diagnostics. We track the effective dimension of the centred activation covariance. At each checkpoint we also build the graph Laplacian $L_t$ and derive the regularised inverse, or resolvent covariance, $\Sigma_t = (L_t + \varepsilon I)^{-1}$ on the retained non-trivial spectral subspace, an inversion which emphasises low-frequency structure. We measure the Bures--Wasserstein distance between consecutive Gaussians.}
\end{enumerate}

The spatial \addedtext{analysis} captures what the representation is \emph{doing}, by tracking how activation vectors move between checkpoints. \addedtext{The global diagnostics capture how the representation and its diffusion geometry evolve.}
...
\begin{itemize}
    ...
    \item \addedtext{Reproducibility: for each of eight runs, the data seed is also supplied to \texttt{torch.manual\_seed} before model creation; there is no separate model seed. The runs use seeds 598, 599, 777, 1001, 2027, 3141, 4096, and 8080, and are pinned to TransformerLens~2.17.0.}
\end{itemize}
...
\]
where $\mathrm{d} f$ is exact (the gradient of a 0-form), $\delta g$ is coexact (the codifferential of a 2-form), and $h$ is harmonic. The three components are orthogonal on a smooth manifold. On a finite point cloud the numerical cross terms are small but non-zero. The raw magnitude charts are very noisy, so to interpret them we look at the composition rather than the raw magnitudes. We report normalised energy fractions $(\hat{e}_{\mathrm{exact}}, \hat{e}_{\mathrm{coexact}}, \hat{e}_{\mathrm{harmonic}})$ that sum to one by construction, telling us how much of the probes' motion is gradient-like, coexact-like, or left in the residual harmonic component. We read the harmonic term conservatively in this \addedtext{paper}: without an independent persistent homology or Betti number calculation we cannot treat the harmonic energy as direct evidence for a topological cycle.

\addedtext{For the robustness analysis we use eight independently trained seeds, four random 400-example probe subsets, and the same nine preselected checkpoint pairs. Starting from the baseline setting of PCA dimension 10, $k$NN${}=15$, and 50 basis functions, we vary one setting at a time over PCA dimensions 5 and 20, $k$NN values 10 and 20, and 30 and 75 basis functions. This gives seven settings and 56 correlated seed-by-setting sensitivity comparisons; the eight training seeds, rather than those 56 comparisons, are the independent units for inference. The primary fixed convention labels pair midpoints before 1{,}500 as pre-grokking, 1{,}500--4{,}000 inclusive as transition, and later midpoints as post-grokking. As a phase-definition sensitivity check, we anchor each seed at its first saved checkpoint with validation accuracy at least 0.5, define its transition as $[-1{,}500,+1{,}000]$ epochs around that event, and reclassify the same nine pairs (yielding two to five pairs per phase, depending on seed). At the baseline parameter setting, we draw one separate destination-point permutation for each seed, probe subset, and checkpoint pair: $8\times4\times9=288$ permutations. Actual and null fractions are paired within the same training seed, probe subset, and checkpoint pair, averaged within seed, and then analysed across the eight seeds. All reported 95\% intervals are two-sided Student $t$ intervals over eight seed-level means ($7$ degrees of freedom). Finally, we calibrate the implementation on synthetic gradient, rotational, and discrete harmonic fields.}
...
\addedtext{The quantitative Hodge analysis within this paper uses eight independently trained seeds. We also ran probe robustness checks on the Hodge fractions and on the global corroborating diagnostics. Across four random 400-example subsets, the Hodge phase shift, dimensional collapse, and BW spiking at the transition point survive.}

\addedtext{The Hodge fractions remain sensitive to PCA dimension, neighbourhood range, and spectral cutoff. The completed one-at-a-time sweep supports the direction of the post-grokking shift, but not universal coexact dominance during the transition or universal exact dominance after it. The eight runs reach 99\% validation accuracy between epochs 3{,}500 and 5{,}500; the directional result survives both the fixed and seed-relative phase definitions, although neither exhausts reasonable phase conventions. The conclusions therefore remain bound to this architecture, task, and finite parameter grid.}
...
\subsubsection{\addedtext{Eight-Seed Robustness and Correspondence Null}}

\addedtext{Table~\ref{tab:eight-seed-hodge} summarises the baseline Hodge composition using training seeds as the independent units.}

\begin{table}[htbp]
\centering
\caption{\addedtext{Baseline Hodge energy fractions across eight training seeds. Values are means of the seed-level phase means.}}
\label{tab:eight-seed-hodge}
\begin{tabular}{lccc}
\toprule
\addedtext{Phase} & \addedtext{Exact} & \addedtext{Coexact} & \addedtext{Harmonic} \\
\midrule
\HodgeBaselineRows
\bottomrule
\end{tabular}
...
\addedtext{The reported $\HodgeFixedChange$ estimate is the baseline-setting contrast, not a pool over parameter settings. Within each seed we first average over its four probe subsets and selected checkpoint pairs, calculate $(\mathrm{coexact}-\mathrm{exact})_{\mathrm{post}}-(\mathrm{coexact}-\mathrm{exact})_{\mathrm{transition}}$, and then average the resulting eight seed contrasts. Its two-sided seed-level $t$ interval is $[\HodgeFixedChangeLow,\HodgeFixedChangeHigh]$. All \HodgeFixedNegativeComparisons/\HodgeSensitivityComparisons{} correlated fixed-window seed-by-setting changes are negative. After seed-relative phase alignment the baseline estimate is $\HodgeEventChange$ ($[\HodgeEventChangeLow,\HodgeEventChangeHigh]$), and all \HodgeEventNegativeComparisons/\HodgeSensitivityComparisons{} sensitivity changes remain negative. Absolute post-grokking exact dominance occurs in \HodgeFixedPostExactComparisons/\HodgeSensitivityComparisons{} fixed-window comparisons and \HodgeEventPostExactComparisons/\HodgeSensitivityComparisons{} seed-relative comparisons.}

\begin{table}[htbp]
\centering
\caption{\addedtext{Phase-definition sensitivity of the baseline contrast. The estimate and 95\% interval use eight seed-level values; the two rightmost columns are descriptive counts of correlated sensitivity comparisons.}}
\label{tab:hodge-phase-sensitivity}
\begin{tabular}{lcccc}
\toprule
\addedtext{Phase definition} & \addedtext{Mean change} & \addedtext{95\% seed-level $t$ interval} & \addedtext{Negative} & \addedtext{Post exact-leading} \\
\midrule
\HodgePhaseSensitivityRows
\bottomrule
\end{tabular}
\end{table}

\addedtext{The correspondence null is based on 288 separately drawn permutations, one for each training-seed--probe-subset--checkpoint-pair baseline record. Each actual field is paired with its null at the same seed, subset, and checkpoint pair before within-seed averaging. Post-grokking, the actual-minus-null exact fraction is $\HodgePostNullExact$ (95\% seed-level $t$ interval $[\HodgePostNullExactLow,\HodgePostNullExactHigh]$), while the coexact difference is $\HodgePostNullCoexact$ ($[\HodgePostNullCoexactLow,\HodgePostNullCoexactHigh]$).}

\addedtext{The synthetic calibration assigns 93.2\% of a gradient field to the exact component, 98.8\% of a rotational field to the coexact component, and 94.2\% of a discrete harmonic field to the harmonic component. Across the empirical sweep, the median between-subset standard deviations are 0.015 exact, 0.020 coexact, and 0.015 harmonic.}

\subsection{\addedtext{Global Corroborating Diagnostics}}
...
The key insight here is that memorisation and generalisation have distinct geometric signatures. The difference, therefore, is not between geometric and non-geometric storage. It is between what is stored geometrically: individual data points (memorisation) versus an algorithmic structure (generalisation). Grokking is the phase transition between these two modes of memory. \addedtext{The BW framework and Hodge decomposition employed in this paper delineate different aspects of the transition period, and together they illustrate algorithmic reorganisation in a neural network.}

\section{Conclusion}

\addedtext{The primary contribution is a pointwise velocity-field Hodge decomposition across eight grokking runs. The effective dimension of the centred activation covariance collapses from 50.7 to 6.4 in the baseline run, while the BW distance between successive resolvent operators shows a large initial spike, a smaller transition-time increase, and then remains in a smaller range once grokking is over. The completed Hodge sweep shows a transition-to-post shift toward exact velocity in all eight seeds and all \HodgeSensitivityComparisons{} correlated seed-by-setting sensitivity comparisons. At the baseline setting, the seed-level mean change in coexact-minus-exact is $\HodgeFixedChange$, and the correspondence null removes the post-grokking exact enrichment. Since absolute exact dominance occurs in \HodgeFixedPostExactComparisons/\HodgeSensitivityComparisons{} fixed-window comparisons, the supported conclusion is directional: generalisation is followed by movement toward a more exact velocity field, not by a universal component ordering in every run.}

\appendix

\section{\addedtext{Supplementary Geometric Diagnostics}}
...
\subsection{\addedtext{Heat Kernel Multiscale Analysis}}
...
\addedtext{Eight trained seeds; 99\% validation accuracy occurs between epochs 3{,}500 and 5{,}500.} \\
\addlinespace[0.2em]
\addedtext{Point motion shifts toward exact velocity after grokking} &
\addedtext{At the baseline setting, mean coexact-minus-exact changes by $\HodgeFixedChange$ from transition to post-grokking.} &
\addedtext{Eight independent seeds; four probe subsets; seven settings; \HodgeFixedNegativeComparisons/\HodgeSensitivityComparisons{} correlated directional sensitivity comparisons; seed-relative alignment; synthetic calibration; and a 288-permutation paired null.} &
\addedtext{Absolute post-grokking exact dominance occurs in \HodgeFixedPostExactComparisons/\HodgeSensitivityComparisons{} fixed-window comparisons; finite phase definitions and parameter grid.} \\
\addlinespace[0.2em]
...
\Cref{tab:grokking-claims-evidence} also identifies what would weaken our interpretation. The analysis on the operator side held under three independent training seeds for the dimensionality collapse and the BW spikes during the grokking transition. However, it could still lose its persuasiveness if those effects disappeared under held-out probe resampling. \addedtext{The Hodge decomposition now survives the completed eight-seed, probe, and one-at-a-time parameter sweeps in its directional claim. The circular-coordinate diagnostics remain supplementary and do not receive the same robustness claim.}
