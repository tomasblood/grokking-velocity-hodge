...
\begin{abstract}
Grokking refers to the phenomenon of neural networks hitting near-perfect training accuracy early and then taking orders of magnitude more steps to generalise on a held-out dataset \citep{power2022grokking}. The contribution is a geometric description across three grokking runs. The spatial analysis showed that, in the baseline seed, the probe velocity evolves from a coexact-dominated (curl-like, $\sim$49\%) regime during memorisation to an exact-dominated (directed, $\sim$69\%) post-grokking regime, with the exact component rising during the transition window (epochs 1{,}500 to 4{,}000). Across three seeds the coexact fraction in this window is 0.48--0.56, peaking at the start of the transition in two of them. The operator side captures how the \emph{geometry itself} evolves, by tracking how the diffusion structure on the manifold changes shape. Further, a stronger mechanistic claim would also require the Hodge decomposition and circular coordinate diagnostics to survive parameter and probe sweeps, which we were not able to complete due to resource constraints.
\end{abstract}
...
The contribution is a geometric description across three grokking runs. The transition is clearly visible in the representation and in the operator geometry. The stronger mechanistic readings (Fourier circle, Hodge composition) appear during the transition window but vary across seeds outside it. We audit the evidential status of these claims in \Cref{tab:grokking-claims-evidence}.

In this chapter we study the grokking transition through two complementary lenses. Both are built on a shared representation space made from activation checkpoints.

\begin{enumerate}
    \item Spatial side. The probe displacement field $v_i(t) = x_i(t + \Delta t) - x_i(t)$ between consecutive checkpoints is a vector field on the point cloud at time~$t$. We treat it as a degree-1 object and perform Hodge decomposition via DiffusionGeometry, partitioning the velocity into exact (gradient), coexact (curl), and harmonic components.

    \item Operator side. At each checkpoint we build the graph Laplacian $L_t$ and derive two SPD operator surrogates from it: the regularised inverse, or resolvent covariance, $\Sigma_t = (L_t + \varepsilon I)^{-1}$ on the retained non-trivial spectral subspace, an inversion which emphasises low-frequency structure, and the heat kernel $H_t(\tau) = e^{-\tau L_t}$, which provides a multiscale family parameterised by diffusion time~$\tau$. For consistency, the heat-kernel BW analysis is computed on the same retained non-trivial spectral subspace unless otherwise stated. For each operator we measure the Bures--Wasserstein distance between consecutive Gaussians.
\end{enumerate}

The spatial side captures what the representation is \emph{doing}, by tracking how activation vectors move between checkpoints. The operator side captures how the \emph{geometry itself} evolves, by tracking how the diffusion structure on the manifold changes shape.
...
\paragraph{Operator side: heat kernel BW at multiple scales.} The complementary operator-level view replaces the resolvent covariance with the heat kernel
\[
H_t(\tau) = e^{-\tau L_t},
\]
...
The quantitative analysis within this chapter is a three-seed case study, and any other run was used only to check our results; those runs sit outside the main pipeline. We also ran a probe robustness check on the cheapest core methods: effective dimensionality, resolvent BW distance and heat-kernel BW distance. Across four random 400-example subsets, the dimensional collapse, the BW spiking at the transition point and the heat-kernel increase across local, meso, and global diffusion scales all survive.

The Hodge decomposition and the circular coordinate scan operate on the same static set of 500 probe examples. Because resampling probes could affect consistency, uncertainty remains around how stable these results are. A more thorough robustness check using repeated probing was tried, along with varying key settings in the Hodge process of the pipeline. These attempts ran into hard limits on memory capacity before finishing. Incomplete due to resource constraints, those expanded tests appear here without full data coverage.

The results of this chapter should be read as findings from three carefully analysed training runs. Changing the configurations slightly can alter the figures even when trends look stable. Our conclusions therefore remain bound to the setup used and nothing broader.

Lastly, the ridge $R^2$ values reported below serve as in-sample diagnostics on a consistent set of probes. With $n < d$, the initialisation score rises artificially due to overfitting and should therefore not be read as pointing to real Fourier structure. Since the DG circular value tracks the best pair among numerous pairs of eigenfunctions, it serves better as a tool to spot top-performing pairs rather than persistent canonical coordinates. As noted above, the Hodge fractions shift with changes to the PCA dimension, along with adjustments to both neighbourhood range and the spectral cutoff used. All results below come from one configuration and three seeds rather than a sweep of a larger seed bank.
...
\subsection{BW Distance Between Geometric States}
...
\subsection{Operator Side: Heat Kernel Multiscale Analysis}
