"""Recover circular coordinates from representation geometry."""
...
def main() -> None:
    ...
    P = CONFIG.modulus
    KNN = CONFIG.knn
    N_BASIS = CONFIG.circular_basis
    PCA_DIM = CONFIG.circular_pca_dim
    N_EIGPAIRS = CONFIG.circular_eigenpairs
    ...
    ax = axes[0, 0]
    ax.plot(
        epochs_arr, corr_f, "o-", color=MAIN_COLOR, markersize=3, linewidth=2.0, label="Fourier-supervised"
    )
    ax.plot(
        epochs_arr, corr_d, "s-", color=ACCENT_COLOR, markersize=3, linewidth=2.0, label="DG unsupervised"
    )
    ...
    # Panels 1 0 and 1 1 PCA scatter at a transition and final epoch
    # coloured by DG circular coordinate

    for col, ep_show in enumerate([2500, EPOCHS[-1]]):
        ax = axes[1, col]
    ...
    print(
        "fourier corr range",
        min(row["corr_fourier"] for row in qa_rows),
        max(row["corr_fourier"] for row in qa_rows),
    )
