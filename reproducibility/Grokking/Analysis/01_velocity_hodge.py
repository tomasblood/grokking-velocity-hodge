"""Decompose pointwise representation velocity into Hodge components."""
...
def main() -> None:
    ...
    PCA_DIM = CONFIG.hodge_pca_dim
    PCA_SOLVER = CONFIG.pca_solver
    KNN = CONFIG.knn
    N_BASIS = CONFIG.hodge_basis
    ...
    fig, axes = plt.subplots(
        2, 1, figsize=(8.2, 5.8), height_ratios=[3, 1], sharex=True, gridspec_kw={"hspace": 0.08}
    )
    ax = axes[0]
    ...
    ax2.set_xlim(0, CONFIG.max_epoch + 5 * CONFIG.save_every)
    ...
    PAIRS = [
        (CONFIG.transition_start, CONFIG.transition_start + CONFIG.save_every),
        (CONFIG.transition_start + 2 * CONFIG.save_every, CONFIG.transition_start + 3 * CONFIG.save_every),
        (CONFIG.transition_end, CONFIG.transition_end + CONFIG.save_every),
    ]
    ROW_LABELS = [
        f"Pre-grokking\n({PAIRS[0][0]} \u2192 {PAIRS[0][1]})",
        f"Transition\n({PAIRS[1][0]} \u2192 {PAIRS[1][1]})",
    ...
    N_SHOW = CONFIG.hodge_quiver_points
        ...
        for component_row, (V_2d, col_label, qcol, frac) in enumerate(
            ...
            if component_row > 0:
                ax.text(
                    ...
                    va="bottom",
                    color=qcol,
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec=qcol, alpha=0.85),
                )
    ...
    for row in qa_payload["pairs"]:
        ...
        if CONFIG.transition_start <= midpoint <= CONFIG.transition_end:
